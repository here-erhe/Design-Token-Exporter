...
const exportTokens = (selectedLayers, type) => {
    ...
    if (selectedCount !== 0) {
        ...
        if (_.size(variables) == 0) {
            sketch.UI.alert(
                'Select layers or artboard',
                'Select one artboard or multiple layers'
            );
        } else {
            ...
            let file = NSString.stringWithString(
                formatObject(variables, type, 'props')
            );
...
export default () => {
    ...
    if (selectedCount !== 0) {
        ...
        sketch.UI.alert(
            'Select tokens',
            'Please select layers or artboard first.'
        );
    }
};
